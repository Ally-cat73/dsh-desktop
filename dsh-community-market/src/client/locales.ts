export const zh = {
  tab: '插件市场',
  title: '社区插件市场',
  subtitle: '从你选择的来源发现与 Aera Code 兼容的插件',
  close: '关闭',
  closeMarket: '关闭插件市场',
  discover: '发现',
  installable: '可安装',
  ...
  manualInstallBody: '以下命令由 Aera Code 根据插件信息生成，可能与仓库中的说明不同。请先检查源码，再复制到 Aera Code 终端执行。',
  installCommand: '安装命令',
  ...
  terminalError: '无法打开 Aera Code 终端，请从应用菜单手动打开后再复制命令。',
  restartError: '无法请求重启 Aera Code，请稍后手动重启。',
  installableBody: '这里显示当前来源中提供 npm 安装目标的插件。安装前会从 npm 获取最新稳定版并确认它是兼容插件。',
  ...
  desktopRequiredTitle: '需要 Aera Code',
  desktopUnavailable: '安装和卸载只在 Aera Code 中可用；你仍然可以浏览插件目录。',
  ...
  installFailedBody: 'Aera Code 未能完成自动安装。你可以查看详细输出，或前往 Aera Code 终端手动安装。',
  installFailureTerminalHint: '自动安装失败后不会再次使用同一个确认请求。请检查下面的输出；如需继续，可以打开 Aera Code 终端并手动执行安装命令。',
  failureOutput: '详细输出',
  noFailureOutput: '没有收到 package manager 的额外输出。请查看 Aera Code 日志了解更多信息。',
  plugin: '插件',
  package: 'npm 包',
  exactVersion: '精确版本',
  profile: '当前配置',
  ...
  confirmInstallBody: '请确认 Aera Code 验证的 npm 包、版本和目标配置。',
  ...
  restartAfterOperation: '操作完成后需要重启 Aera Code，改动才会生效。',
  disabledRestartRequired: '此插件保留了未加载状态；插件市场仍只提供卸载。',
  installComplete: '插件安装完成',
  uninstallComplete: '插件卸载完成',
  restartRequiredTitle: '需要重启 Aera Code',
  restartRequiredBody: '请重启 Aera Code，让当前配置加载最新的插件状态。',
  restartLater: '稍后重启',
  restartNow: '立即重启',
  restarting: '正在重启...',
  done: '完成',
...
export const en: Record<MarketLocaleKey, string> = {
  ...
  subtitle: 'Discover Aera Code-compatible plugins from sources you choose',
  close: 'Close',
  ...
  manualInstallBody: 'Aera Code generated this command from the plugin information, so it may differ from the repository instructions. Review the source first, then copy it into Aera Code Terminal.',
  ...
  terminalError: 'Aera Code Terminal could not be opened. Open it from the application menu, then copy the command manually.',
  restartError: 'Aera Code could not be restarted. Restart it manually when convenient.',
  installableBody: 'This view shows source entries with an npm install target. Before installation, Aera Code resolves npm latest and confirms that it is a compatible plugin.',
  ...
  immutablePlugin: 'Core runtime component',
  ...
  desktopRequiredTitle: 'Aera Code is required',
  desktopUnavailable: 'Install and uninstall are available only in Aera Code. You can still browse the catalog.',
  ...
  installFailedBody: 'Aera Code could not complete the automatic installation. Review the detailed output or continue manually in Aera Code Terminal.',
  installFailureTerminalHint: 'Aera Code will not reuse the same confirmation after a failed automatic install. Review the output below; to continue, open Aera Code Terminal and run the install command manually.',
  failureOutput: 'Detailed output',
  noFailureOutput: 'The package manager returned no additional output. Check the Aera Code logs for more information.',
  plugin: 'Plugin',
  package: 'npm package',
  ...
  confirmInstallBody: 'Review the npm package, version, and target profile verified by Aera Code.',
  ...
  restartAfterOperation: 'Restart Aera Code after this operation for the change to take effect.',
  ...
  restartRequiredTitle: 'Restart Aera Code',
  restartRequiredBody: 'Restart Aera Code so the current profile loads its updated plugin state.',
