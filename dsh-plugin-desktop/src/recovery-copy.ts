...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    ...
    rollbackBody: 'Choose one of the three healthy-start slots to restore the current Profile together with shared settings.yaml and the runtime patch.',
    plugins: 'Plugin management',
    pluginsBody: 'Remove a direct plugin dependency from the current Profile with Aera Code’s built-in plugin command.',
    ...
    configurationFilesBody: 'View or edit the current Profile and shared runtime configuration. Restart Aera Code after making changes.',
    ...
    confirmUninstallBody: 'Aera Code will remove this dependency from the current Profile and reconcile its plugin layers. This does not depend on which market installed it.',
    confirmRollback: 'Roll back this configuration?',
    confirmRollbackBody: capturedAt => `This immediately restores the current Profile plus the checkpointed settings.yaml and runtime patch captured at ${capturedAt}. After restarting, Aera Code will use the rolled-back configuration.`,
    ...
    operationStageLabels: {
      ...
      'plugin-change': 'Aera Code plugin uninstall',
    },
  ...
  zh: {
    ...
    rollbackBody: '从三个健康启动槽位中选择一个，同时恢复当前 Profile、共享 settings.yaml 和运行时补丁。',
    plugins: '插件管理',
    pluginsBody: '使用 Aera Code 内置的插件命令，从当前 Profile 中卸载直接依赖的插件。',
    ...
    configurationFilesBody: '查看或编辑当前 Profile 与共享运行时配置。修改后需要重新启动 Aera Code。',
    ...
    confirmUninstallBody: 'Aera Code 将从当前 Profile 中移除此依赖，并重新整理插件层。无论插件由哪个市场安装，都会使用相同的卸载流程。',
    confirmRollback: '回滚这套配置？',
    confirmRollbackBody: capturedAt => `将立即恢复 ${capturedAt} 创建的 Checkpoint 中的当前 Profile、settings.yaml 与运行时补丁；重启后，Aera Code 将使用回滚后的配置。`,
